...
buttonSelfComment.on('click', function () {
  var roomId = buttonSelfComment.data('room-id');
  var userId = buttonSelfComment.data('user-id');
  var comment = prompt('コメントを255字以内で入力してください。');
  if (comment) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().post("/rooms/".concat(roomId, "/users/").concat(userId, "/comments"), {
      comment: comment
    }, function (data) {
...
var buttonDeleteMusic = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#delete-btn');
buttonDeleteMusic.on('click', function () {
  var roomId = buttonDeleteMusic.data('room-id');
  var musicId = buttonDeleteMusic.data('music-id');
  var result = confirm('削除してもよろしいですか？');
  if (result) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().post("/rooms/".concat(roomId, "/music/").concat(musicId, "?delete=1"), {
      result: result
    });
  }
});
